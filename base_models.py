...
from dataloader.augmentations import DataTransform_time

def channel_separation(time, dataset_name='SleepEDFx'):
    if dataset_name == 'SleepEDFx':
        EEG_channels = time[:-1, :]
        EOG_channel = time[-1, :]
        EOG_channel = EOG_channel[np.newaxis, ...]
        ...
        assert acc_channels.shape == (3, 128)
        assert gyro_channels.shape == (3, 128)
        return acc_channels, gyro_channels

class SSLDataSet(Dataset):
    def __init__(self, data, dataset_name='SleepEDFx'):
...
class FTDataSet(Dataset):
    def __init__(self, data, label, dataset_name='SleepEDFx', multi_label=False):
    ...
    def __len__(self):
        # print(f"DEBUG: self.data type in len is {type(self.data)}")
