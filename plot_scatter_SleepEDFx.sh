...
#SBATCH --job-name=plot_scatter_SleepEDFx
#SBATCH --array=1-5
#SBATCH --output=logs_plot_scatter/SleepEDFx_%A_%a.out
...
python -u scripts/plot_binary_gmm_pair_scatter.py \
    ...
    --seed "${SEED}" \
    --checkpoint_path checkpoints_baseline_best/best_setup_4000_4_warmup4_SleepEDFx \
    --out_dir plots_scatter/SleepEDFx \
    --epochs 10 \
    --warm_epochs 4 \
    --batch_size 128 \
    --lr 1e-3 \
