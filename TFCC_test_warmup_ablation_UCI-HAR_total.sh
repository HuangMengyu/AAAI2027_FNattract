...
srun --output="logs_warmup_ablation/test/UCI-HAR_total/warmup${warm_epochs}_UCI-HAR_total_%A_%a.out" \
python test.py \
    --dataset_name UCI-HAR_total \
    --baseline_checkpoint_path checkpoints_baseline_best/baseline_UCI-HAR_total \
    --checkpoint_path checkpoints/warmup_ablation/warmup${warm_epochs}_UCI-HAR_total \
