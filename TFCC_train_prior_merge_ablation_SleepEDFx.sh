...
#SBATCH --job-name=prior_merge_ablation_SleepEDFx
#SBATCH --array=1-15
...
# For Testing:
srun --output="logs_prior_merge_ablation/SleepEDFx/4000_4_warmup4_prior${prior_delta_mode}_SleepEDFx_%A_%a.out" \
python -u main_5fold.py \
    --dataset_name SleepEDFx \
    --current_num_fold ${current_num_fold} \
    --epochs 10 \
    --warm_epochs 4 \
    --model_save_path checkpoints/prior_merge_ablation/4000_4_warmup4_prior${prior_delta_mode}_SleepEDFx \
    --batch_size 128 \
    --lr 1e-3 \
    --ssl True \
    --use_prior True \
